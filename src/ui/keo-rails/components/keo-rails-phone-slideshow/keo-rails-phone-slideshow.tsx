...
function BottomNav({ active }: { active: string }) {
  return (
    <div className={styles.bottomNav}>
      <div className={`${styles.navItem} ${active === "dashboard" ? styles.navItemActive : ""}`}>
        <Grid3x3 />
        <span>Dashboard</span>
      </div>
      <div className={`${styles.navItem} ${active === "invoices" ? styles.navItemActive : ""}`}>
        <Receipt />
        <span>Invoices</span>
      </div>
      <div className={`${styles.navItem} ${active === "account" ? styles.navItemActive : ""}`}>
        <User />
        <span>Account</span>
      </div>
    </div>
    ...
    >
      ...
      <div className={styles.floatingIcons}>
        {screens.map((screen, index) => {
          ...
          const position = index === 0 ? "left" : index === 1 ? "bottom" : "right";

          return (
            <motion.div
              key={screen.id}
              className={`${styles.floatingIcon} ${
                styles[`icon${position.charAt(0).toUpperCase() + position.slice(1)}`]
              }`}
