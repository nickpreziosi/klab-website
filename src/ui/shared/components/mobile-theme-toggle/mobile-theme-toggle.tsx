...
export function MobileThemeToggle() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("themechange", onExternalThemeChange as EventListener);
    window.addEventListener("storage", onExternalThemeChange as EventListener);

    return () => {
      window.removeEventListener("themechange", onExternalThemeChange as EventListener);
      window.removeEventListener("storage", onExternalThemeChange as EventListener);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const handleSystemThemeChange = (event: MediaQueryListEvent) => {
      if (event.matches) {
        ...
        document.documentElement.style.setProperty("--main-color-rgb", "250, 250, 250");
      ...
      <Accordion.Item value="theme" className={styles.accordionItem}>
        ...
        <Accordion.Content className={styles.accordionContent} asChild forceMount>
          <motion.div
            ...
            animate={open === "theme" ? { height: "auto", opacity: 1 } : { height: 0, opacity: 0 }}
          ...
          >
            <div className={styles.themeOptions}>
              {["light", "dark", "system"].map((themeName) => (
                ...
                >
                  <span className={styles.optionIcon}>{getThemeIcon(themeName)}</span>
                  <span className={styles.optionLabel}>{getThemeLabel(themeName)}</span>
                </button>
              ))}
            </div>
          </motion.div>
