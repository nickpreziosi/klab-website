...
function CircleLogo() {
  const uid = "krails-" + useId().replace(/:/g, "-");
  return (
    <svg
...
export default function KRailsCaseStudy() {
  ...
  useEffect(() => {
    ...
    const sync = () => {
      ...
      if (leftApi && leftApi.selectedScrollSnap() !== idx) leftApi.scrollTo(idx);
      if (introApi && introApi.selectedScrollSnap() !== idx) introApi.scrollTo(idx);
    };
          ...
          <div className={`${styles.embla} embla`} ref={introRef}>
            <div className={`${styles.embla__container}`}>
              {caseStudyData.map((cs, idx) => {
                ...
                return (
                  <div className={styles.embla__slide} key={cs.id} data-case={cs.id}>
                    <motion.div
                      ...
                      >
                        {cs.highlights.map((highlight, index) => (
                          <div className={styles.highlight} key={index}>
                            <h3 className={styles.highlightHeading}>{highlight.heading}</h3>
        ...
        <div className={styles.mobileControls}>
          <button
            ...
            onClick={() => {
              ...
              const prev = (current - 1 + caseStudyData.length) % caseStudyData.length;
